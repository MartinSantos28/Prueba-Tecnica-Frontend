...
export async function fetchAllUsers() : Promise<User[]> {
  const first = await fetchUsers(1, 10)

  if (first.total <= first.users.length) {
    const pages = Math.ceil (first.total / 100)
    const rest = await Promise.all(
      Array.from({ length : pages -1 } , (_, i) => fetchUsers(i + 2, 100))
    )
    return [first.users , ...rest.flatMap(r => r.users)].flat();
  }
} 


export async function fetchAllProducts() : Promise<Product[]> { 
  const first = await fetchProducts(1, 10)
  ...
  if (first.total <= first.products.length) { 
    ...
    return [first.products , ...rest.flatMap(r => r.products)].flat()
  }
}